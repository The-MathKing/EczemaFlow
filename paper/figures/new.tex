...
\\
\textbf{Results:} We present \softwarename, a conditional flow-matching model that combines pathology-image features with persistent-homology summaries of nuclei point clouds and Fourier-encoded spatial coordinates, validated on 6 Visium slides (15,281 spots) from a mixed inflammatory skin disease cohort (GSE206391). Using 6-fold cross-validation, the spatially anchored flow architecture without topology achieves the best predictive accuracy among evaluated models (MSE $0.329 \pm 0.006$) outperforming both CNN regressors and Hist2ST. Adding persistent-topology features unexpectedly worsened MSE (to $0.342$) and reduced correlation, providing no evidence that explicit nuclei topology adds predictive value in this cohort. Furthermore, the base Gaussian flow provides well-calibrated distributional uncertainty (94.6\% nominal interval coverage), whereas the full ZINB flow is undercalibrated (83.2\% coverage).
...
\softwarename~successfully formulates spatial transcriptomics prediction as a continuous normalizing flow. Our empirical evaluations demonstrate that the flow-matching formulation provides highly calibrated credible intervals (94.6\% coverage for Gaussian Flow), representing a significant step forward for uncertainty-aware spatial transcriptomics. However, strict ablation studies demonstrate that explicit topological conditioning does not improve point-prediction performance over standard spatial-image features. While the No Topology Flow ablation achieves the best Mean Squared Error (0.329) among evaluated models on a mixed inflammatory skin disease cohort, the inclusion of multiscale topology slightly degraded this accuracy.
