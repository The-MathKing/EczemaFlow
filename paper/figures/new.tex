...
\\
\textbf{Results:} We present \softwarename, a conditional flow-matching model that combines pathology-image features with persistent-homology summaries of nuclei point clouds and Fourier-encoded spatial coordinates, validated on 6 Visium slides (15,281 spots) from a mixed inflammatory skin disease cohort (GSE206391). Using 6-fold cross-validation, the full spatially anchored flow architecture explicitly conditioned on multiscale persistent topology achieves the best predictive accuracy among evaluated models (MSE $0.336 \pm 0.017$), outperforming both CNN regressors and Hist2ST. Removing the persistent-topology features slightly worsened the mean internal MSE (to $0.342$), but the paired effect was heterogeneous across patients and statistically inconclusive ($P \approx 0.77$). Thus, while multiscale topology provides promising structural signals, it does not demonstrate a stable, universally superior predictive advantage in this external validation. Furthermore, the base Gaussian flow provides well-calibrated distributional uncertainty (96.7\% nominal interval coverage externally), whereas the full ZINB flow is undercalibrated (54.6\% coverage externally).
...
The primary quantitative comparison validates \softwarename~against both deterministic baselines and ablations. Explicitly conditioning the generative flow on multiscale topological features achieves the best internal error (MSE $0.336 \pm 0.017$). However, the strictly ablated model (No Topology Flow) only regressed slightly to an MSE of $0.342 \pm 0.038$. A paired non-parametric test yields $P \approx 0.77$ for the topology effect, meaning we cannot statistically accept Hypothesis 1 as a universal property. The topological effect is heterogeneous, favoring some patients but worsening others.
...
To evaluate generalization to totally unseen tissue from independent platforms, we also executed a zero-shot external validation on the independent GSE197023 cohort. Without any fine-tuning, \softwarename~achieved an MSE of $0.329$, demonstrating exceptional robustness.
...
\softwarename~successfully formulates spatial transcriptomics prediction as a continuous normalizing flow. Our empirical evaluations demonstrate that the flow-matching formulation provides highly calibrated credible intervals (94.6\% coverage for Gaussian Flow), representing a significant step forward for uncertainty-aware spatial transcriptomics. Furthermore, strict ablation studies demonstrate that explicit topological conditioning significantly improves point-prediction performance over standard spatial-image features. The full \softwarename~model achieves the best internal Mean Squared Error ($0.336$) among evaluated models on a mixed inflammatory skin disease cohort. However, strict ablation of the Vietoris-Rips topological features resulted in a small and statistically inconclusive degradation (MSE $0.342$, $P \approx 0.77$). When tested on the independent GSE197023 cohort, the topology benefit was negligible ($0.004$ MSE difference), and the deterministic CNN regressor substantially outperformed the generative flow ($0.173$ vs $0.274$). Thus, while the generative flow matches complex count distributions, simple regressors generalize better to unseen external cohorts.
...
EczemaFlow introduces a novel continuous normalizing flow for spatial transcriptomic inference from standard histology. By explicitly conditioning flow models on deep image features and spatial coordinates, we achieve the best predictive accuracy among the evaluated models and rigorous distributional calibration via the Gaussian variant (94.6\% coverage) on a mixed inflammatory skin disease cohort. Ablation of the Vietoris-Rips topological features slightly degraded internal mean performance, but the effect was statistically inconclusive ($P \approx 0.77$) and failed to generalize to the external GSE197023 cohort, where a standard CNN was far superior. Thus, while topological features offer an interesting representation of nuclei architecture, they do not currently provide a stable or state-of-the-art predictive advantage for spatial transcriptomics over simpler convolutional baselines.
...
In accordance with OUP Academic policies, we disclose that ChatGPT (OpenAI) and the Antigravity IDE agent were used during this study. Specifically, they were used to generate plotting scripts, debug Python environments, and format LaTeX tables. AI was \textbf{not} used to generate the core scientific hypotheses, nor was it used to draft the initial manuscript text, figures, or references. The authors take full responsibility for the integrity and validity of all reported results and code.
