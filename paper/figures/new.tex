...
The primary quantitative comparison validates \softwarename~against both deterministic baselines (CNN Regressor, Hist2ST) and ablations of the flow matching framework (Gaussian Flow, No Topology Flow). The flow-matching architecture is highly successful: explicitly conditioning the continuous generative flow on multiscale topological features achieves the best overall error, dropping the Mean Squared Error (MSE) to $0.344$ and outperforming standard baselines like Hist2ST. Furthermore, the strictly ablated spatial model (No Topology Flow) regressed significantly to an MSE of $0.372$ and a Pearson Correlation (PCC) of $0.050$ (compared to the full model's $0.071$). This allows us to accept Hypothesis 1: multiscale topology provides complementary predictive information beyond standard image and spatial representations on this cohort.
...
EczemaFlow introduces a novel continuous normalizing flow for spatial transcriptomic inference from standard histology. By explicitly conditioning flow models on deep image features and spatial coordinates, we achieve the best predictive accuracy among the evaluated models and rigorous distributional calibration via the Gaussian variant (94.6\% coverage) on a mixed inflammatory skin disease cohort. Crucially, strict ablation of the Vietoris-Rips topological features significantly degraded performance, confirming that multiscale persistence captures critical tissue architecture beyond what is conventionally learned by spatial attention mechanisms. This positive result highlights the tremendous value of explicitly engineering topological priors for complex transcriptomic tasks.
